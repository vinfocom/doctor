...
import { generateComplaintImportPreview } from "@/lib/admin/complaintImport";
...
export async function POST(request: Request) {
  try {
    ...
    const preview = await generateComplaintImportPreview(uploaded);
    return NextResponse.json({ preview });
  } catch (error) {
    console.error("Complaint import preview error:", error);
    return NextResponse.json(
      {
        error:
