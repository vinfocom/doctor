...
import { generateDiagnosisImportPreview } from "@/lib/admin/diagnosisImport";
...
export async function POST(request: Request) {
  try {
    ...
    const preview = await generateDiagnosisImportPreview(uploaded);
    return NextResponse.json({ preview });
  } catch (error) {
    console.error("Diagnosis import preview error:", error);
    return NextResponse.json(
      {
        error:
