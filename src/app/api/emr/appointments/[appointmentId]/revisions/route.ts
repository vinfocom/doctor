...
async function loadAppointmentForRevision(appointmentId: number) {
...
function buildRevisionResponse(
  appointment: NonNullable<Awaited<ReturnType<typeof loadAppointmentForRevision>>>,
  draft: NonNullable<Awaited<ReturnType<typeof createPrescriptionRevisionDraft>>>
) {
  return {
    context: {
...
export async function POST(
  ...
  try {
    ...
    const appointment = await loadAppointmentForRevision(parsedAppointmentId);
    ...
    const editReason = String(body.editReason ?? "");
    ...
    if (!editReason.trim()) {
      throw new EmrAccessError(400, "Edit reason is required");
    }
    ...
    if (!draft) {
      return NextResponse.json(
        { error: "Failed to create prescription revision" },
        { status: 500 }
      );
    }

    return NextResponse.json(buildRevisionResponse(appointment, draft), {
      status: 200,
    });
  } catch (error) {
    ...
    logEmrOperationalError("emr-revision", error);

    return NextResponse.json(
      {
        error: getDoctorSafeErrorMessage(
          error,
          "Failed to create prescription revision"
        ),
      },
      { status: 500 }
    );
