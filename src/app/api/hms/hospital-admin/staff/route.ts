...
export async function GET(req: Request) {
    try {
        ...
        const [staff, doctors] = await Promise.all([
            prisma.$queryRawUnsafe<StaffRow[]>(
                `
                SELECT
                    hs.staff_id,
                    u.user_id,
                ...
                WHERE hs.hospital_id = ?
                  AND hs.staff_type = 'REGISTRATION'
                GROUP BY
                    hs.staff_id,
                    u.user_id,
                    u.name,
        ...
        console.error("List HMS registration staff error:", error);
...
export async function POST(req: Request) {
    try {
        ...
        if (!name) fieldErrors.name = "Staff name is required.";
        if (name.length > 255) fieldErrors.name = "Name must be 255 characters or fewer.";
        ...
        const doctorRows = await prisma.$queryRawUnsafe<DoctorOptionRow[]>(
            ...
            INNER JOIN doctors d
             ...
             AND d.admin_id = ?
            WHERE hd.hospital_id = ?
              AND hd.doctor_id IN (${doctorPlaceholders})
            `,
            hospital.adminId,
            hospital.hospitalId,
            ...doctorIds
        ...
        if (doctorRows.length !== doctorIds.length) {
            return NextResponse.json(
                { error: "One or more selected doctors do not belong to this hospital.", fieldErrors: { doctor_ids: "Select only doctors from this hospital." } },
                { status: 400 }
            );
        }
        ...
        const account = await prisma.$transaction(async (tx) => {
            ...
            if (!userId) throw new Error("Registration staff user was not created.");

            await tx.$executeRawUnsafe(
                `
                INSERT INTO hospital_staff (hospital_id, user_id, staff_type, status, valid_from, valid_to)
            ...
            if (!staffId) throw new Error("Registration staff row was not created.");

            for (const doctorId of doctorIds) {
                await tx.$executeRawUnsafe(
                    `
                    INSERT INTO hospital_staff_doctor_access (staff_id, doctor_id)
                    VALUES (?, ?)
                    `,
                    staffId,
                    doctorId
        ...
        console.error("Create HMS registration staff error:", error);
        return NextResponse.json({ error: "Unable to create registration staff. Please try again." }, { status: 500 });
    }
}
