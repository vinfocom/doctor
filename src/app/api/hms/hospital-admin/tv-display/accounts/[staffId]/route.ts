...
export async function PUT(req: Request, { params }: { params: Promise<{ staffId: string }> }) {
    try {
        return await updateStaff(req, params, "TV_DISPLAY");
    } catch (error) {
        console.error("Update HMS TV display account error:", error);
        return NextResponse.json({ error: "Unable to update TV display account." }, { status: 500 });
    }
}

export async function DELETE(req: Request, { params }: { params: Promise<{ staffId: string }> }) {
    try {
        return await deleteStaff(req, params, "TV_DISPLAY");
    } catch (error) {
        console.error("Delete HMS TV display account error:", error);
        return NextResponse.json({ error: "Unable to delete TV display account." }, { status: 500 });
    }
}
