...
type MedicineImportPreview = {
  ...
  groups: Record<ImportStatus, MedicineImportRow[]>;
};
...
type SectionConfig<TPreview, TRow> = {
  ...
  mapImportRows: (preview: TPreview) => unknown[];
  buildSuccessMessage: (result: ImportResult) => string;
  buildConfirmTitle: () => string;
  buildConfirmBody: (preview: TPreview) => string;
  searchText: (row: TRow) => string;
  columns: Array<{
    header: string;
    accessorKey: (row: TRow) => ReactNode;
  }>;
};
...
function formatStatusLabel(status: ImportStatus) {
  return TAB_CONFIG.find((item) => item.key === status)?.label ?? status;
}

function StatusBadge({ status }: { status: ImportStatus }) {
  const tone =
    TAB_CONFIG.find((item) => item.key === status)?.tone ??
...
function ImportSection<TPreview extends { file_name: string; generated_at: string; summary: ImportSummary; groups: Record<ImportStatus, TRow[]> }, TRow>({
  config,
}: {
  config: SectionConfig<TPreview, TRow>;
}) {
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  const [preview, setPreview] = useState<TPreview | null>(null);
  const [activeTab, setActiveTab] = useState<ImportStatus>("will_import");
  ...
  const currentTabRows = useMemo(() => {
    ...
    return baseRows.filter((row) =>
      config.searchText(row).toLowerCase().includes(query)
    );
  }, [activeTab, config, preview, search]);
  ...
  const handleGeneratePreview = async () => {
    ...
    try {
      ...
      setSearch("");
      setSuccessMessage("Preview generated. No database rows have been inserted yet.");
    } catch (fetchError) {
      setPreview(null);
      setError(
  ...
  const handleDownloadReport = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `${preview.file_name.replace(/\.[^.]+$/, "") || config.filePrefix}_preview_report.xlsx`;
  ...
  const handleImport = async () => {
    ...
    try {
      const response = await fetch(config.importEndpoint, {
        ...
        body: JSON.stringify({
          rows: config.mapImportRows(preview),
        }),
      });
      ...
      if (!response.ok || !data.result) {
        throw new Error(data.error || "Could not import rows.");
      }

      setImportResult(data.result);
      setSuccessMessage(config.buildSuccessMessage(data.result));
      setShowConfirmModal(false);
    } catch (importError) {
      setError(
        importError instanceof Error ? importError.message : "Could not import rows."
      );
    } finally {
      setImporting(false);
    }
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div className="flex-1">
            ...
            <div className="rounded-2xl border border-dashed border-gray-300 bg-gray-50 p-4">
              ...
              />
              <p className="mt-3 text-xs text-gray-500">{config.uploadHelp}</p>
            </div>
          </div>
        ...
        <>
          ...
          <GlassCard className="space-y-5 p-5">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <h3 className="text-lg font-bold text-gray-900">Preview Results</h3>
            ...
            <PremiumTable columns={config.columns} data={currentTabRows} />
          </GlassCard>
        </>
      ) : null}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 px-4">
          <div className="w-full max-w-lg rounded-3xl bg-white p-6 shadow-2xl">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="text-xl font-bold text-gray-900">{config.buildConfirmTitle()}</h3>
                <p className="mt-2 text-sm leading-6 text-gray-600">
                  {config.buildConfirmBody(preview)}
                </p>
              </div>
            </div>
...
const medicineColumns = [
  ...
  {
    header: "Status",
    accessorKey: (row: MedicineImportRow) => <StatusBadge status={row.status} />,
  },
  {
    header: "Reason",
    accessorKey: (row: MedicineImportRow) => (
...
const testColumns = [
  ...
  {
    header: "Status",
    accessorKey: (row: TestImportRow) => <StatusBadge status={row.status} />,
  },
  {
    header: "Reason",
    accessorKey: (row: TestImportRow) => (
    ...
    <div className="mx-auto max-w-7xl space-y-10 px-4 py-6 sm:px-6 sm:py-8">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm text-gray-600 sm:text-base">
            Upload `.xlsx` or `.csv` files, generate a read-only preview, review invalid or duplicate rows,
      ...
      <ImportSection<MedicineImportPreview, MedicineImportRow>
        config={{
          title: "Medicine Bulk Import",
          description:
            "Upload a medicine `.xlsx` or `.csv`, review the preview, and import only clean rows into the medicines master table.",
          uploadHelp:
            "Required column: name. Optional columns: type, strength value, strength unit, salt composition, company.",
          filePrefix: "medicine_import",
          previewEndpoint: "/api/admin/medicines/import/preview",
          reportEndpoint: "/api/admin/medicines/import/report",
          importEndpoint: "/api/admin/medicines/import",
          ...
          buildSuccessMessage: (result) =>
            `Import completed. ${result.inserted_count} new medicines were inserted.`,
          buildConfirmTitle: () => "Confirm Medicine Import",
          buildConfirmBody: (preview) =>
            `This will insert ${preview.summary.will_import} new medicines into the master table. Rows marked as Needs Review, Invalid, Duplicate In File, and Already Exists will not be imported.`,
          searchText: (row) =>
            [
              row.name,
              row.normalized_name,
      ...
      <ImportSection<TestImportPreview, TestImportRow>
        config={{
          title: "Tests Bulk Import",
          description:
            "Upload a tests `.xlsx` or `.csv`, review pending or rejected collisions carefully, and import only the rows that are safe to add to the tests master table.",
          uploadHelp:
            "Required column: name. Accepted aliases include test name, test_name, test, and investigation.",
          filePrefix: "test_import",
          previewEndpoint: "/api/admin/tests/import/preview",
          reportEndpoint: "/api/admin/tests/import/report",
          importEndpoint: "/api/admin/tests/import",
          mapImportRows: (preview) =>
            preview.groups.will_import.map((row) => ({
              name: row.name,
              normalized_name: row.normalized_name,
          ...
          buildSuccessMessage: (result) =>
            `Import completed. ${result.inserted_count} new tests were inserted.`,
          buildConfirmTitle: () => "Confirm Tests Import",
          buildConfirmBody: (preview) =>
            `This will insert ${preview.summary.will_import} new tests into the master table. Rows marked as Needs Review, Invalid, Duplicate In File, and Already Exists will not be imported.`,
          searchText: (row) =>
