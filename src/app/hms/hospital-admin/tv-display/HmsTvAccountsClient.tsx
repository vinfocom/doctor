...
import { Check, Copy, KeyRound, Loader2, MonitorPlay, Pencil, Plus, Power, PowerOff, RefreshCw, Trash2, X } from "lucide-react";
...
type ConfirmAction =
    | { type: "RESET"; account: TvAccount }
    | { type: "DELETE"; account: TvAccount }
    | { type: "TOGGLE"; account: TvAccount };
...
function isAccountActive(account: TvAccount) {
...
function doctorIdsFromAccount(account: TvAccount) {
    return (account.assigned_doctors || [])
        .map((doctor) => Number(doctor?.doctor_id))
        .filter((id) => Number.isInteger(id) && id > 0);
}

function isExpiredByValidTo(account: TvAccount) {
    const validTo = toInputDate(account.valid_to);
...
export default function HmsTvAccountsClient() {
    ...
    const sortedAccounts = useMemo(() => {
        return [...accounts].sort((left, right) => String(left.name || "").localeCompare(String(right.name || "")));
    }, [accounts]);
    ...
    const loadAccounts = useCallback(async () => {
        ...
        try {
            const res = await fetch("/api/hms/hospital-admin/tv-display/accounts", { cache: "no-store" });
            const data = await res.json().catch(() => ({}));

            if (!res.ok) {
                setError(data.error || "Unable to load TV display accounts.");
                return;
            }
            ...
            setError("Unable to load TV display accounts. Check your connection and try again.");
        } finally {
            setLoading(false);
        }
    }, []);
    ...
    const startEdit = (account: TvAccount) => {
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        setSubmitting(true);
        try {
            const res = await fetch(editingStaffId ? `/api/hms/hospital-admin/tv-display/accounts/${editingStaffId}` : "/api/hms/hospital-admin/tv-display/accounts", {
            ...
            if (!res.ok) {
                setError(data.error || "Unable to create TV display account.");
                setFieldErrors(data.fieldErrors || {});
                return;
            }

            setAccounts((prev) => editingStaffId
                ? prev.map((account) => account.staff_id === editingStaffId ? data.account : account)
                : [...prev, data.account]);
            setForm(emptyForm);
            setEditingStaffId(null);
            setFieldErrors({});
            ...
            setSuccess(editingStaffId ? "TV display account updated." : "TV display account created. Copy the temporary password now.");
        } catch {
            setError(`Unable to ${editingStaffId ? "update" : "create"} TV display account. Check your connection and try again.`);
        } finally {
            setSubmitting(false);
        }
    };

    const resetPassword = async (account: TvAccount) => {
        setError("");
        setSuccess("");
        setTemporaryPassword("");
        setCopiedPassword(false);
    ...
    const deleteAccount = async (account: TvAccount) => {
        setActingStaffId(account.staff_id);
        setError("");
        setSuccess("");
        try {
            const res = await fetch(`/api/hms/hospital-admin/tv-display/accounts/${account.staff_id}`, { method: "DELETE" });
            const data = await res.json().catch(() => ({}));
            if (!res.ok) {
                setError(data.error || "Unable to delete TV display account.");
                return;
            }
            setAccounts((prev) => prev.filter((item) => item.staff_id !== account.staff_id));
            setSuccess("TV display account deleted.");
        } catch {
            setError("Unable to delete TV display account. Check your connection and try again.");
        } finally {
            setActingStaffId(null);
        }
    };

    const toggleAccount = async (account: TvAccount) => {
        const active = isAccountActive(account);
        setActingStaffId(account.staff_id);
        setError("");
        setSuccess("");
        try {
            const res = await fetch(`/api/hms/hospital-admin/tv-display/accounts/${account.staff_id}`, {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
            ...
            const data = await res.json().catch(() => ({}));
            if (!res.ok) {
                setError(data.error || "Unable to update TV display status.");
                return;
            }
            setAccounts((prev) => prev.map((item) => item.staff_id === account.staff_id ? data.account : item));
            setSuccess(active ? "TV display account deactivated." : "TV display account activated.");
        } catch {
            setError("Unable to update TV display status. Check your connection and try again.");
        } finally {
            setActingStaffId(null);
        }
    };
        ...
        <div className="w-full">
            <div className="mb-6 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                <div>
                    ...
                    <h1 className="mt-1 text-2xl font-bold text-gray-950 sm:text-3xl">TV Display</h1>
                </div>
                <button
                    type="button"
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
                    <div className="w-full max-w-md rounded-lg border border-black bg-white shadow-xl">
                        <div className="flex items-start justify-between gap-3 border-b border-black px-4 py-3">
                            <div>
                                <p className="text-xs font-semibold uppercase tracking-wide text-black">Temporary Password</p>
                                <h2 className="mt-1 text-base font-semibold text-black">Copy this password now</h2>
                            </div>
                            <button
                                type="button"
                        ...
                        <div className="px-4 py-4">
                            <p className="mb-3 text-sm text-black">
                                Share this with the TV display operator. It will not be shown again after you close this popup or refresh the page.
                            </p>
                            <div className="flex items-center gap-2">
                                <code className="min-w-0 flex-1 rounded-lg border border-black bg-white px-3 py-2 text-sm font-semibold text-black">
                                    {temporaryPassword}
                                </code>
                                <button
                                    type="button"
                                    ...
                                    className="inline-flex items-center justify-center gap-2 rounded-lg bg-black px-3 py-2 text-sm font-semibold text-white hover:bg-black"
                                >
            ...
            {confirmAction && (
                <ConfirmModal
                    title={confirmAction.type === "RESET" ? "Reset password?" : confirmAction.type === "DELETE" ? "Delete TV account?" : isAccountActive(confirmAction.account) ? "Deactivate TV account?" : "Activate TV account?"}
                    message={confirmAction.type === "RESET" ? "A new temporary password will be shown once." : confirmAction.type === "DELETE" ? "This TV display login and doctor access will be removed." : isAccountActive(confirmAction.account) ? "This account will not be able to log in." : "This account will be active from today if the valid dates were expired."}
            ...
            <div className="grid items-start gap-6 xl:grid-cols-[minmax(0,1fr)_390px]">
                <div className="overflow-hidden rounded-lg border border-gray-200 bg-white">
                    <div className="border-b border-gray-100 px-4 py-3">
                        <h2 className="text-sm font-semibold text-gray-950">Accounts</h2>
                    </div>
                    {loading ? (
                        ...
                        <div className="px-4 py-6 text-sm text-gray-500">No TV display accounts.</div>
                    ) : (
                                        ...
                                        <tr key={account.staff_id} className="border-t border-gray-100">
                                            <td className="px-4 py-3">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        <p className="font-semibold text-gray-950">{account.name || `TV ${account.staff_id}`}</p>
                                            ...
                                            <td className="px-4 py-3 text-gray-600">
                                                {formatDate(account.valid_from)} to {formatDate(account.valid_to)}
                                            </td>
                                            <td className="px-4 py-3">
                ...
                <form onSubmit={handleSubmit} className="rounded-lg border border-gray-200 bg-white p-4">
                    <div className="mb-4 flex items-center gap-2">
                        ...
                        <h2 className="text-sm font-semibold text-gray-950">{editingStaffId ? "Edit TV Account" : "Create TV Account"}</h2>
                    ...
                    <div className="space-y-4">
                        ...
                        <div>
                            <label className="mb-2 block text-xs font-medium text-gray-600">
                                <HmsLabelWithInfo
                                    label="Doctors *"
                                    info="The TV screen will rotate queues only for the selected doctors."
                                />
                            </label>
                    ...
                    >
                        ...
                        {editingStaffId ? "Save Account" : "Create Account"}
                    </button>
                </form>
            </div>
        </div>
