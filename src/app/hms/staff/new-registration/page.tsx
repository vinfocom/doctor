...
export default async function HmsStaffNewRegistrationPage() {
    ...
    if (!(await isHmsFeatureEnabled(session.hospitalContext, "reception_module"))) {
        return <HmsFeatureDisabled title="New Registration" />;
    }

    const preRegistrationEnabled = await isHmsFeatureEnabled(session.hospitalContext, "qr_temp_token_enabled");

    return <HmsReceptionDashboardClient mode="registration" preRegistrationEnabled={preRegistrationEnabled} />;
}
