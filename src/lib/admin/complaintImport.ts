...
const HEADER_ALIASES: Record<string, string[]> = {
  name: ["name", "complaint name", "complaint_name", "complaint", "symptom"],
};

export type ComplaintImportStatus =
  | "will_import"
  | "already_exists"
  | "duplicate_in_file"
  | "invalid"
...
export type ComplaintImportRow = {
  row_number: number;
  name: string;
  normalized_name: string;
  status: ComplaintImportStatus;
  reasons: string[];
  source: {
    raw_name: string;
  };
...
export type ComplaintImportSummary = {
  total_rows: number;
  will_import: number;
...
export type ComplaintImportPreview = {
  file_name: string;
  generated_at: string;
  summary: ComplaintImportSummary;
  rows: ComplaintImportRow[];
  groups: Record<ComplaintImportStatus, ComplaintImportRow[]>;
};
...
type ImportableComplaintRow = Pick<ComplaintImportRow, "name" | "normalized_name">;
...
async function loadExistingComplaintsByNormalizedNames(normalizedNames: string[]) {
  const existing = new Map<string, ExistingComplaintRow>();
  const batchSize = 500;
  ...
  for (let index = 0; index < normalizedNames.length; index += batchSize) {
    ...
    const rows = await prisma.$queryRaw<ExistingComplaintRow[]>(
      Prisma.sql`
...
function buildSummary(rows: ComplaintImportRow[]): ComplaintImportSummary {
  return rows.reduce<ComplaintImportSummary>(
    (summary, row) => {
...
function groupRowsByStatus(rows: ComplaintImportRow[]) {
  return rows.reduce<Record<ComplaintImportStatus, ComplaintImportRow[]>>(
    (groups, row) => {
...
function createBaseRow(parsedRow: ParsedSheetRow): ComplaintImportRow {
  ...
  let status: ComplaintImportStatus = "will_import";

  if (!rawName) {
    status = "invalid";
    reasons.push("Complaint name is blank.");
  } else if (!normalizedName) {
    status = "invalid";
    reasons.push("Complaint name could not be normalized.");
  } else if (name.length > 255) {
    status = "invalid";
    reasons.push("Complaint name exceeds the 255 character limit.");
  }
...
function classifyRows(
  rows: ComplaintImportRow[],
  existingByNormalizedName: Map<string, ExistingComplaintRow>
) {
  const rowsByNormalized = new Map<string, ComplaintImportRow[]>();
  ...
  rows.forEach((row) => {
    ...
    if (existing.status === "approved") {
      ...
      row.reasons.push(
        `Already exists in database as approved complaint "${existing.name}" (ID ${existing.id}).`
      );
      return;
    }
    ...
    row.status = "needs_review";
    row.reasons.push(
      `Matches existing ${existing.status} complaint "${existing.name}" (ID ${existing.id}).`
    );
  });
...
export async function generateComplaintImportPreview(
  file: File
): Promise<ComplaintImportPreview> {
  const parsedRows = await parseSpreadsheetFile(file);
  const baseRows = parsedRows.map(createBaseRow);
  const normalizedNames = Array.from(
    new Set(baseRows.map((row) => row.normalized_name).filter(Boolean))
  );
  const existingByNormalizedName = await loadExistingComplaintsByNormalizedNames(
    normalizedNames
  );
...
export async function importComplaintRows(input: {
  rows: ImportableComplaintRow[];
  chunkSize?: number;
}) {
  const dedupedRowsMap = new Map<string, ImportableComplaintRow>();
  ...
  const existingBeforeImport = await loadExistingComplaintsByNormalizedNames(
  ...
  for (let index = 0; index < rowsToInsert.length; index += chunkSize) {
    ...
    const affected = await prisma.$executeRaw(
      Prisma.sql`
        INSERT IGNORE INTO complaints_master (
          name,
          normalized_name,
          status,
...
export async function buildComplaintImportReportWorkbook(
  preview: ComplaintImportPreview
) {
  const workbook = new ExcelJS.Workbook();
  ...
  const sheetConfigs: Array<[string, ComplaintImportStatus]> = [
